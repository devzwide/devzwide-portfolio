...
const Content = () => {
  return (
    <div className="p-3 sm:p-4 md:p-8">
      <div className="border-l-2 border-gray-300 pl-4 sm:pl-6 md:pl-8 space-y-8 md:space-y-12">
        {timeline.map((item) => (
          <div key={item.year} className="relative group">
            {/* Year bubble with hover effect */}
            <div className="absolute -left-8 sm:-left-9 md:-left-11 top-0 bg-white text-gray-800 rounded-full w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 flex items-center justify-center font-semibold text-xs sm:text-sm md:text-base shadow-md border-2 border-gray-300 group-hover:bg-blue-50 group-hover:border-blue-400 group-hover:text-blue-600 transition-all duration-200">
              {item.year}
            </div>
            
            {/* Content card with subtle shadow on hover */}
            <div className="ml-2 p-3 sm:p-4 rounded-lg bg-white shadow-sm group-hover:shadow-md transition-shadow duration-200">
              <div className="mb-2 font-bold text-base sm:text-lg text-gray-800">
                {item.title}
              ...
              {item.quote && (
                <blockquote className="italic text-gray-600 mb-3 pl-2 sm:pl-3 border-l-2 border-gray-200">
                  “{item.quote}”
                </blockquote>
              )}
        ...
        <div className="relative group">
          <div className="absolute -left-8 sm:-left-9 md:-left-11 top-0 bg-blue-100 text-blue-600 rounded-full w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 flex items-center justify-center font-semibold shadow-md border-2 border-blue-200 transition-all duration-200">
            →
          </div>
          
          <div className="ml-2 p-3 sm:p-4 rounded-lg bg-blue-50 border border-blue-100">
            <div className="mb-2 font-bold text-base sm:text-lg text-blue-800">
              {future.title}
            </div>
            
            {future.quote && (
              <blockquote className="italic text-blue-600 mb-3 pl-2 sm:pl-3 border-l-2 border-blue-200">
                “{future.quote}”
              </blockquote>
            )}
      ...
      <div className="mt-8 md:mt-12 p-4 sm:p-5 bg-gray-50 border-l-4 border-blue-400 rounded-lg shadow-sm">
        <div className="font-bold text-base sm:text-lg mb-2 text-gray-800 flex items-center">
          <svg className="w-5 h-5 mr-2 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        </div>
        <p className="text-gray-700 pl-1 text-sm sm:text-base">"{summary}"</p>
      </div>
    </div>
  );
};
...
const Journey = () => {
  return (
    <motion.div className="p-3 sm:p-4 md:p-8">
